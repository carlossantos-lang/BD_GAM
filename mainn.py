...
SHEET_NAME = "BD - GAM"
DATE_STRING = "2025-09-28"   # Pode automatizar depois com datetime.date.today()
...
for d in DOMAINS:
    ...
    for row in data:
        ...
        all_rows.append([
            ...
            requests_val,
            f"{revenue:.2f}".replace(".", ","),                 # Revenue com vírgula
            "" if match_rate == 0 else f"{match_rate:.4f}".replace(".", ","),  # Match rate opcional
            f"{ecpm:.2f}".replace(".", ",")                     # eCPM com vírgula
        ])
